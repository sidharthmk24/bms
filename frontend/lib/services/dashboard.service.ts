...
export class DashboardService {
  ...
  async getBranchInventoryDashboard(user: JwtPayload) {
    ...
    `, [user.branchId, user.branchId, user.branchId, user.branchId]);
  ...
  async getBranchFrontOfficeDashboard(user: JwtPayload) {
    ...
    const [stats] = await dataSource.manager.query(`
      SELECT 
        (SELECT SUM(total_amount) FROM bill WHERE branch_id = ? AND status = 'COMPLETED' AND DATE(created_at) = CURDATE()) as todaySales,
        (SELECT SUM(total_amount) FROM bill WHERE branch_id = ? AND status = 'COMPLETED' AND payment_mode = 'CASH' AND DATE(created_at) = CURDATE()) as cashSales,
        (SELECT SUM(total_amount) FROM bill WHERE branch_id = ? AND status = 'COMPLETED' AND payment_mode = 'UPI' AND DATE(created_at) = CURDATE()) as upiSales,
    ...
    `, [user.branchId, user.branchId, user.branchId, user.branchId, user.branchId, user.branchId]);
