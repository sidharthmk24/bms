...
import { DollarSign, TrendingDown, TrendingUp, AlertCircle, Loader2, Calendar, LayoutDashboard, BarChart2 } from 'lucide-react';
import Link from 'next/link';
import {
  AreaChart, Area, BarChart, Bar, LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip as RechartsTooltip, ResponsiveContainer, Legend
} from 'recharts';
...
export default function FinanceDashboard() {
  ...
  const [plChartType, setPlChartType] = useState<'line' | 'bar'>('line');
  const [branchChartType, setBranchChartType] = useState<'bar' | 'line'>('bar');
        ...
        <>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center space-x-3">
              <Link 
                ...
                className="inline-flex items-center justify-center rounded-sm bg-[#7e2562] px-4 py-2 text-xs font-semibold text-white shadow-sm hover:bg-[#681b50] transition-colors"
              >
                Manage Expenses
              </Link>
              <div className="w-40">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <StatCard 
              ...
              icon={AlertCircle} 
              color={dashboardData?.discrepancies > 0 ? 'red' : 'plum'} 
            />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  {plChartType === 'line' ? (
                    ...
                    <BarChart data={dashboardData?.trendData || []}>
                      ...
                      />
                      <Legend />
                      <Bar dataKey="revenue" name="Revenue" fill="#3cb976" radius={[2, 2, 0, 0]} />
                      <Bar dataKey="expense" name="Expense" fill="#e45e34" radius={[2, 2, 0, 0]} />
                    </BarChart>
                  )}
                </ResponsiveContainer>
              </div>
            ...
            <div className="bg-white p-5 rounded-sm border border-neutral-200/80 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                ...
                <div className="inline-flex rounded-sm border border-[#7e2562]/20 bg-[#faf6f9] p-0.5 shadow-2xs">
                  <button
                    onClick={() => setBranchChartType('bar')}
                    className={`flex items-center gap-1.5 px-2.5 py-1 text-xs font-bold rounded-xs transition-all cursor-pointer ${
                      branchChartType === 'bar'
                        ? 'bg-white text-[#7e2562] shadow-xs'
                        : 'text-neutral-500 hover:text-neutral-900'
                  ...
                  <button
                    onClick={() => setBranchChartType('line')}
                    className={`flex items-center gap-1.5 px-2.5 py-1 text-xs font-bold rounded-xs transition-all cursor-pointer ${
                      branchChartType === 'line'
                        ? 'bg-white text-[#7e2562] shadow-xs'
                        : 'text-neutral-500 hover:text-neutral-900'
                    }`}
                    title="Line View"
                ...
                <ResponsiveContainer width="100%" height="100%">
                  {branchChartType === 'bar' ? (
                    ...
                    <LineChart data={branchData || []}>
                      ...
                      />
                      <Legend />
                      <Line type="monotone" dataKey="revenue" name="Revenue" stroke="#7e2562" strokeWidth={2.5} dot={{ r: 4 }} />
                      <Line type="monotone" dataKey="expenses" name="Expenses" stroke="#e45e34" strokeWidth={2.5} dot={{ r: 4 }} />
                    </LineChart>
                  )}
                </ResponsiveContainer>
              </div>
