...
export class FinanceService {
  constructor(
    ...
    @InjectRepository(ExpenseRevision)
    private readonly expenseRevisionRepo: Repository<ExpenseRevision>,
    @InjectRepository(CashReconciliation)
    private readonly cashReconRepo: Repository<CashReconciliation>,
  ...
  async createCashReconciliation(dto: CreateCashReconciliationDto, user: JwtPayload): Promise<CashReconciliation> {
    ...
    const recon = this.cashReconRepo.create({
    ...
    try {
      return await this.cashReconRepo.save(recon);
    } catch (err: any) {
  ...
  async getBranchComparison(startDate: string, endDate: string): Promise<any[]> {
    const branches = await this.dataSource.manager.query(`SELECT id, name FROM branch`);
    
    const revenue = await this.dataSource.manager.query(
